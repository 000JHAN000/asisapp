...
import { UbicacionOrmEntity } from 'src/sede/infrastructure/entities/ubicacion.orm-entity';
...
@Injectable()
export class UbicacionesCGService {
  constructor(
  ...
  private async getRepo() {
    return this.connectionManager.getTenantRepository(this.tenantId, UbicacionOrmEntity);
  }
  ...
  async findOne(id: string) {
    const repo = await this.getRepo();
    return repo.findOne({ where: { id_ubicacion: id } });
  }

  async create(data: Partial<UbicacionOrmEntity>) {
    const repo = await this.getRepo();
    const entity = repo.create(data);
    return repo.save(entity);
  }

  async update(id: string, data: Partial<UbicacionOrmEntity>) {
    const repo = await this.getRepo();
