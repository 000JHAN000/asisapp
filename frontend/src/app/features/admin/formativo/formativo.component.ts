...
type TabKey =
  | 'centros' | 'sedes' | 'departamentos' | 'municipios' | 'ambientes' | 'ubicaciones'
          ...
          @for (f of tabConfig[activeTab()].fields; track f.key) {
            ...
            @if (f.type === 'select') {
              <app-ss [options]="getSSOpts(f.opts!)"
                      ...
                      (ngModelChange)="onFieldChange(f.key, $event)"></app-ss>
              <!-- Hint para cursos: el área debe seleccionarse antes del ambiente -->
              @if (activeTab() === 'cursos' && f.key === 'ambiente_id' && !formData['area_id']) {
...
export class AdminFormativoComponent implements OnInit {
  ...
  tabGroups: TabGroup[] = [
    { label: 'Estructura Organizacional', icon: 'building', tabs: ['centros', 'sedes', 'departamentos', 'municipios', 'ambientes', 'ubicaciones'] },
  ...
  tabConfig: Record<TabKey, {
    ...
    centros: {
      ...
      fields: [{ key: 'nombre', label: 'Nombre', required: true, span2: true }],
    ...
    sedes: {
      ...
      fields: [
        ...
        { key: 'centro_id', label: 'Centro de Formación', type: 'select', opts: 'centros', required: true },
      ],
    ...
    municipios: {
      ...
      fields: [
        { key: 'nombre', label: 'Nombre', required: true },
        { key: 'departamento_id', label: 'Departamento', type: 'select', opts: 'departamentos', required: true },
      ],
    ...
    ubicaciones: {
      label: 'Ubicaciones', desc: 'Auditorios, bibliotecas y otros espacios de la sede',
      cols: [
        { key: 'id', label: 'ID' }, { key: 'nombre', label: 'Nombre' },
      ...
      fields: [
        { key: 'nombre',    label: 'Nombre',    required: true, span2: true },
        { key: 'tipo',      label: 'Tipo',      type: 'enum-tipo-ubicacion', required: true },
        { key: 'capacidad', label: 'Capacidad', type: 'number' },
        { key: 'area_id',   label: 'Área',      type: 'select', opts: 'areas' },
        { key: 'sede_id',   label: 'Sede',      type: 'select', opts: 'sedes' },
      ],
      load: () => this.api.getUbicaciones().subscribe({ next: r => this.rows.set(r), error: e => this.setErr(e) }),
      create: d => this.api.createUbicacion(d), update: (id, d) => this.api.updateUbicacion(id, d), remove: id => this.api.deleteUbicacion(id),
    },
    programas: {
    ...
    cursos: {
      ...
      fields: [
        ...
        { key: 'programa_id', label: 'Programa', type: 'select', opts: 'programas', required: true },
        { key: 'area_id', label: 'Área', type: 'select', opts: 'areas', required: true },
        { key: 'ambiente_id', label: 'Ambiente Asignado', type: 'select', opts: 'ambientes-disponibles-cursos' },
        { key: 'lider_id', label: 'Instructor Líder', type: 'select', opts: 'instructores-lider-ficha' },
        { key: 'fecha_inicio', label: 'Fecha Inicio', type: 'date', required: true },
        { key: 'fecha_fin', label: 'Fecha Fin', type: 'date', required: true },
        { key: 'fta_lectiva', label: 'Fecha Fin Lectiva', type: 'date' },
      ],
    ...
    matriculas: {
      ...
      fields: [
        { key: 'persona_id', label: 'Persona', type: 'select', opts: 'personas', hidden: true },
        { key: 'curso_id', label: 'Curso', type: 'select', opts: 'cursos', hidden: true },
        { key: 'fecha_matricula', label: 'Fecha de Matrícula', type: 'date', hidden: true },
    ...
    personas: {
      ...
      fields: [
        ...
        { key: 'identificacion', label: 'N° Identificación', type: 'number', required: true },
        ...
        { key: 'municipio_id', label: 'Municipio', type: 'select', opts: 'municipios', required: true },
        { key: 'curso_id', label: 'Asignación de Ficha (Solo para Aprendiz)', type: 'select', opts: 'cursos', hiddenIf: 'cargo !== "aprendiz"' },
    ...
    roles: {
      ...
      fields: [
        ...
        { key: 'aplicativo_id', label: 'Aplicativo', type: 'select', opts: 'aplicativos', required: true },
    ...
    servicios: {
      ...
      fields: [
        { key: 'nombre', label: 'Nombre', required: true },
        { key: 'modulo_id', label: 'Módulo', type: 'select', opts: 'modulos', required: true },
    ...
    usuarios: {
      ...
      fields: [
        { key: 'persona_id', label: 'Persona', type: 'select', opts: 'personas', required: true },
        { key: 'aplicativo_id', label: 'Aplicativo', type: 'select', opts: 'aplicativos', required: true },
    ...
    credenciales: {
      ...
      fields: [
        ...
        { key: 'usuario_id', label: 'Usuario del Sistema', type: 'select', opts: 'usuarios', required: true },
      ],
    ...
    permisos: {
      ...
      fields: [
        { key: 'usuario_id', label: 'Usuario', type: 'select', opts: 'usuarios', required: true },
        { key: 'servicio_id', label: 'Servicio', type: 'select', opts: 'servicios', required: true },
      ],
    ...
    accesos: {
      ...
      cols: [
        { key: 'id', label: 'ID' }, { key: 'usuario_nombre', label: 'Usuario' },
        { key: 'estado', label: 'Estado', badge: true }, { key: 'fecha_creacion', label: 'Inicio' },
        { key: 'fecha_salida', label: 'Salida' },
      ],
      fields: [],
      load: () => this.api.getAccesos(200).subscribe({ next: r => this.rows.set(r), error: e => this.setErr(e) }),
    },
  };
    ...
    switch (listKey) {
      ...
      case 'ambientes-disponibles-cursos': {
        const usados = new Set(
          this.cursos()
            .filter((c: any) => c.ambiente_id && c.id !== this.editingId())
            .map((c: any) => +c.ambiente_id)
        );
        const areaSeleccionada = this.formData['area_id'] ? +this.formData['area_id'] : null;
        return this.ambientes()
          .filter(a => {
            if (usados.has(a.id)) return false;
            // Si hay un área seleccionada, mostrar solo ambientes de esa área
            if (areaSeleccionada) return +a.area_id === areaSeleccionada;
            return true;
          })
          .map(a => ({ id: a.id, label: `${a.nombre}${a.area_nombre ? ' · ' + a.area_nombre : ''}` }));
      }
      ...
      case 'instructores-lider-ficha': {
        const usados = new Set(
          this.cursos()
            .filter((c: any) => c.lider_id && c.id !== this.editingId())
            .map((c: any) => +c.lider_id)
        );
      ...
      case 'instructores-lider-area': {
        const usados = new Set(
          this.areas()
            .filter((a: any) => a.lider_id && a.id !== this.editingId())
            .map((a: any) => +a.lider_id)
        );
        return this.personas()
          .filter((p: any) => p.cargo === 'instructor' && !usados.has(p.id))
          .map((p: any) => ({ id: p.id, label: `${p.nombre} ${p.apellido} — ${p.identificacion}` }));
  ...
  /** Intercepta cambios en selects del modal para lógica dependiente entre campos.
   *  Cuando cambia el área en el formulario de cursos, limpia el ambiente seleccionado
   *  ya que pertenecía a otra área. */
  onFieldChange(fieldKey: string, value: any) {
    if (fieldKey === 'area_id' && this.activeTab() === 'cursos') {
      this.formData['ambiente_id'] = '';
    }
  }
  ...
  openModal(row?: any) {
    ...
    if (row) {
      ...
      const tipoDefault = this.activeTab() === 'ubicaciones' ? 'auditorio' : 'Ambiente';
  ...
  deleteRow(id: number) {
    const warnings: Record<string, string> = {
      ...
      areas: '⚠️ ADVERTENCIA: ¿Eliminar Área Educativa?\nConlleva la eliminación instantánea de los Cursos adscritos a ella.',
