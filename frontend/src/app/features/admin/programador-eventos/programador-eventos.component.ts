      ...
      <div class="flex items-center gap-3" style="flex-wrap:wrap">
        ...
        <div class="form-group" style="min-width:160px">
          ...
          <app-ss [options]="mesOpts" placeholder="Todos los meses"
                  [ngModel]="filterMes()" (ngModelChange)="filterMes.set($event)"></app-ss>
        </div>
      </div>
    </div>
...
export class ProgramadorEventosComponent implements OnInit {
  ...
  formTipo         = signal('');
  filtroArea       = signal('');
  fichasSeleccionadas = signal<Set<number>>(new Set());
  ...
  formUbicacionId     = signal<number | null>(null);
  ...
  calendarCells = computed(() => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const dayEvents = this.eventos().filter(ev => {
  ...
  fichasEnRango = computed(() => {
    ...
    this.horarios().forEach(h => {
      ...
      if (h.horaInicio < hF && h.horaFin > hI) {
        ...
        if (fid) fichaIds.add(+fid);
      }
    });
    return this.fichas().filter(f => fichaIds.has(f.id));
  });
  ...
  irHoy() { this.viewYear = new Date().getFullYear(); this.viewMonth = new Date().getMonth(); }
  ...
  seleccionarTodas() {
    const ids = new Set(this.fichasEnRangoFiltradas().map((f: any) => f.id as number));
    this.fichasSeleccionadas.update(s => {
      const next = new Set(s);
  ...
  openEdit(ev: any) {
    ...
    this.filtroArea.set('');
    this.fichasSeleccionadas.set(new Set((ev.fichasParticipantes ?? []).map(Number)));
    this.formError.set('');
