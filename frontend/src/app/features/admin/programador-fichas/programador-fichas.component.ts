...
@if (selectedFicha() && !loadingFicha()) {
  ...
  @if (activeTab() === 'calendar') {
    ...
    @if (selectedDay()) {
      <div class="day-detail card mt-4">
        ...
        @for (ev of selectedDayEvents(); track ev.comp.id) {
          @let c = ev.comp;
          @let hFull = getFullHorario(c.horarioId);
          ...
          @if (ev.isClassDay) {
            <div class="day-comp-item" [style.border-left-color]="getCompColor(c)">
              ...
              @if (c.resultado) {
                <div style="font-size:11px;color:var(--text-muted);margin-top:2px;line-height:1.4">{{ c.resultado }}</div>
              }
              <!-- Instructor + horario -->
          ...
          <div class="inst-stat-header">
            ...
            <div class="inst-horas-badge"
              ...
              <div>
                ...
                @if (stat.deficit > 0) {
                  <div style="font-size:10px;font-weight:700">−{{ fmtH(stat.deficit) }} déficit</div>
                } @else if (hasTarget) {
      ...
      <div style="display:flex;flex-direction:column;gap:14px">
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" rows="2"
                    [ngModel]="mResultado()" (ngModelChange)="mResultado.set($event)"
        ...
        <div class="form-group">
          ...
          <p class="form-hint">Total de horas que debe cubrir esta competencia según el plan de formación SENA</p>
        </div>
          ...
          <div class="modal-horas-status" [class.mhs-ok]="defM === 0" [class.mhs-deficit]="defM > 0">
            ...
            @if (defM > 0) {
              <p class="mhs-msg mhs-msg-deficit">
                <lucide-icon name="alert-triangle" [size]="12"></lucide-icon>
                Déficit: <strong>{{ fmtH(defM) }}</strong> — se necesitan {{ calcClasesExtra() }} clase(s) adicional(es)
              </p>
            } @else {
...
export class ProgramadorFichasComponent implements OnInit {
  ...
  horarioOpts = computed((): SSOption[] =>
    this.horarios().map(h => ({
      value: h.id,
      label: `${cap(h.diaSemana)} ${h.jornada === 'manana' ? 'Mañana' : 'Tarde'} · ${to12h(h.horaInicio)}–${to12h(h.horaFin)} — ${h.instructor?.nombre ?? ''} ${h.instructor?.apellido ?? ''}`.trim(),
    }))
  );
  ...
  horasStats = computed(() => {
    ...
    this.competencias().forEach(c => {
      const actuales   = Math.round(horasComp(c) * 100) / 100;
      const requeridas = c.horasRequeridas ?? 0;
  ...
  loadFichaData(fichaId: number) {
    ...
    forkJoin({
      ...
      next: ({ horarios, comps }) => {
        this.horarios.set(horarios as any[]);
        const fc = (comps as any[]).filter(
          c => Number(c.horario?.ficha?.id) === Number(fichaId)
        );
  ...
  openEditComp(c: any) {
    ...
    this.mResultado.set(c.resultado ?? '');
    this.mHorarioId.set(c.horarioId);
    this.mFechaInicio.set(toIso(c.fechaInicio));
    this.mFechaFin.set(toIso(c.fechaFin));
    this.mDiasClase.set([...(c.diasClase ?? [])]);
    this.mHorasRequeridas.set(c.horasRequeridas ?? null);
    this.modalOpen.set(true);
    this.cdr.markForCheck(); this.cdr.detectChanges();
  }
  ...
  saveComp() {
    ...
    const dto = {
      horarioId:       this.mHorarioId()!,
      nombre:          this.mNombre().trim(),
      resultado:       this.mResultado().trim(),
  ...
  calcExtDias(): number {
    ...
    const h      = this.horarios().find(x => x.id === c.horarioId) ?? c.horario;
