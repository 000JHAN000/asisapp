...
type TabKey =
  | 'administradores' | 'instructores' | 'aprendices'
  | 'fichas'
...
export class HorariosAdminComponent implements OnInit {
  activeTab = signal<TabKey>('administradores');
  rows      = signal<any[]>([]);
  ...
  fichasOpts: { id: number; label: string }[] = [];

  tabGroups = [
    { label: 'Personal',      icon: 'users',           tabs: ['administradores', 'instructores', 'aprendices'] as TabKey[] },
    { label: 'Académico',     icon: 'book-open',       tabs: ['fichas'] as TabKey[] },
    { label: 'Horarios',      icon: 'calendar',        tabs: ['horarios', 'competencias'] as TabKey[] },
    { label: 'Espacios',      icon: 'building-2',      tabs: ['ambientes', 'eventos'] as TabKey[] },
  ...
  private tabMap: Record<TabKey, TabCfg> = {
    ...
    // ── PERSONAL ──────────────────────────────────────────────
    administradores: {
      label: 'Administradores', desc: 'Cuentas de administrador en horarios_db',
      cols: [
        { key: 'id', label: 'ID' }, { key: 'nombre', label: 'Nombre' }, { key: 'apellido', label: 'Apellido' },
        { key: 'tipo_doc', label: 'Doc' }, { key: 'num_doc', label: 'N° Doc' },
        { key: 'correo', label: 'Correo' }, { key: 'telefono', label: 'Teléfono' },
        { key: 'sesion_activa', label: 'Sesión', bool: true },
      ],
      fields: [
        { key: 'nombre',   label: 'Nombre',     required: true },
        { key: 'apellido', label: 'Apellido',   required: true },
        { key: 'tipo_doc', label: 'Tipo Doc',   type: 'enum-tipo-doc' },
        { key: 'num_doc',  label: 'N° Documento', required: true },
        { key: 'correo',   label: 'Correo',     required: true, span2: true },
        { key: 'telefono', label: 'Teléfono' },
        { key: 'password', label: 'Contraseña', type: 'password', span2: true },
      ],
      load:   () => this.api.getHAdmins().subscribe({ next: r => this.onLoad(r), error: e => this.onErr(e) }),
      create: d => this.api.createHAdmin(d),
      update: (id, d) => this.api.updateHAdmin(id, d),
      remove: id => this.api.deleteHAdmin(id),
    },

    instructores: {
      label: 'Instructores', desc: 'Instructores registrados en horarios_db',
      cols: [
        { key: 'id', label: 'ID' }, { key: 'nombre', label: 'Nombre' }, { key: 'apellido', label: 'Apellido' },
        { key: 'num_doc', label: 'N° Doc' }, { key: 'correo', label: 'Correo' },
        { key: 'es_lider', label: 'Líder', bool: true },
        { key: 'area_liderada', label: 'Área' },
        { key: 'es_transversal', label: 'Trans.', bool: true },
        { key: 'sesion_activa', label: 'Sesión', bool: true },
      ],
      fields: [
        { key: 'nombre',        label: 'Nombre',       required: true },
        ...
        { key: 'genero',        label: 'Género',       type: 'enum-genero' },
        { key: 'es_lider',      label: 'Es Líder',     type: 'enum-bool' },
        { key: 'area_liderada', label: 'Área Liderada' },
        { key: 'es_transversal', label: 'Transversal', type: 'enum-bool' },
        { key: 'password',      label: 'Contraseña',   type: 'password', span2: true },
      ],
      load:   () => this.api.getHInstructores().subscribe({ next: r => this.onLoad(r), error: e => this.onErr(e) }),
      create: d => this.api.createHInstructor(d),
      update: (id, d) => this.api.updateHInstructor(id, d),
      remove: id => this.api.deleteHInstructor(id),
    },

    aprendices: {
      label: 'Aprendices', desc: 'Aprendices registrados en horarios_db',
      cols: [
        { key: 'id', label: 'ID' }, { key: 'nombre', label: 'Nombre' }, { key: 'apellido', label: 'Apellido' },
        { key: 'num_doc', label: 'N° Doc' }, { key: 'correo', label: 'Correo' },
        { key: 'ficha_codigo', label: 'Ficha' }, { key: 'municipio', label: 'Municipio' },
        { key: 'sesion_activa', label: 'Sesión', bool: true },
      ],
      fields: [
        { key: 'nombre',    label: 'Nombre',       required: true },
        { key: 'apellido',  label: 'Apellido',     required: true },
        { key: 'tipo_doc',  label: 'Tipo Doc',     type: 'enum-tipo-doc-apr' },
        { key: 'num_doc',   label: 'N° Documento', required: true },
        { key: 'correo',    label: 'Correo',       required: true, span2: true },
        { key: 'telefono',  label: 'Teléfono' },
        { key: 'ficha_id',  label: 'Ficha',        type: 'select', opts: 'fichas' },
        { key: 'municipio', label: 'Municipio' },
        { key: 'genero',    label: 'Género',       type: 'enum-genero' },
        { key: 'password',  label: 'Contraseña',   type: 'password', span2: true },
      ],
      load:   () => this.api.getHAprendices().subscribe({ next: r => this.onLoad(r), error: e => this.onErr(e) }),
      create: d => this.api.createHAprendiz(d),
      update: (id, d) => this.api.updateHAprendiz(id, d),
      remove: id => this.api.deleteHAprendiz(id),
    },

    // ── ACADÉMICO ─────────────────────────────────────────────
    fichas: {
      label: 'Fichas', desc: 'Fichas de formación',
      cols: [
        { key: 'id', label: 'ID' }, { key: 'codigo', label: 'Código' }, { key: 'programa', label: 'Programa' },
        { key: 'area', label: 'Área' }, { key: 'fecha_inicio', label: 'Inicio' },
        { key: 'fecha_fin', label: 'Fin' }, { key: 'estado', label: 'Estado', badge: true },
      ],
      fields: [
        { key: 'codigo',       label: 'Código',     required: true },
        { key: 'programa',     label: 'Programa',   required: true, span2: true },
        { key: 'area',         label: 'Área' },
        { key: 'estado',       label: 'Estado',     type: 'enum-estado-ficha' },
        { key: 'fecha_inicio', label: 'Fecha Inicio', type: 'date' },
        { key: 'fecha_fin',    label: 'Fecha Fin',    type: 'date' },
      ],
      load:   () => this.api.getHFichas().subscribe({ next: r => this.onLoad(r), error: e => this.onErr(e) }),
      create: d => this.api.createHFicha(d),
      update: (id, d) => this.api.updateHFicha(id, d),
      remove: id => this.api.deleteHFicha(id),
    },
    ...
    // ── ESPACIOS ──────────────────────────────────────────────
    ambientes: {
      label: 'Ambientes', desc: 'Ambientes en horarios_db',
      cols: [
        { key: 'id', label: 'ID' }, { key: 'nombre', label: 'Nombre' },
      ...
      load:   () => this.api.getHAmbientes().subscribe({ next: r => this.onLoad(r), error: e => this.onErr(e) }),
      create: d => this.api.createHAmbiente(d),
      update: (id, d) => this.api.updateHAmbiente(id, d),
      remove: id => this.api.deleteHAmbiente(id),
    },
    ...
    eventos: {
      ...
      cols: [
        { key: 'id', label: 'ID' }, { key: 'nombre', label: 'Nombre' },
        { key: 'tipo', label: 'Tipo' }, { key: 'fecha_inicio', label: 'F. Inicio' },
        { key: 'fecha_fin', label: 'F. Fin' }, { key: 'hora_inicio', label: 'H. Inicio' },
  ...
  private enumMap: Record<string, { value: string; label: string }[]> = {
    'enum-tipo-doc': [
      { value: 'CC', label: 'CC — Cédula' }, { value: 'TI', label: 'TI — Tarjeta Identidad' },
      { value: 'CE', label: 'CE — C. Extranjería' }, { value: 'PA', label: 'PA — Pasaporte' },
    ],
    'enum-tipo-doc-apr': [
      { value: 'TI', label: 'TI — Tarjeta Identidad' }, { value: 'CC', label: 'CC — Cédula' },
      { value: 'CE', label: 'CE — C. Extranjería' }, { value: 'PA', label: 'PA — Pasaporte' },
    ],
    'enum-genero': [
      { value: 'masculino', label: 'Masculino' }, { value: 'femenino', label: 'Femenino' }, { value: 'otro', label: 'Otro' },
    ],
    'enum-bool': [
  ...
  ngOnInit() {
    this.loadTab();
    this.api.getHFichasOpts().subscribe({ next: r => this.fichasOpts = r });
  }
  ...
  openModal(row?: any) {
    ...
    if (row) {
      ...
      this.formData = { tipo_doc: 'CC', genero: 'masculino', es_lider: '0', es_transversal: '0', estado: 'activa', tipo: 'formativo' };
    }
  ...
  getSelectOpts(opts: string) {
    if (opts === 'fichas') return this.fichasOpts;
    return [];
  }
