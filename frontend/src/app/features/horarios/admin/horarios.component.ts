...
@Component({
  ...
  template: `
    ...
    <div class="matrix-wrap mt-4">
      ...
      @if (activeTab() === 'ambiente') {
        <div class="matrix-disp-header" style="display:flex; align-items:center; gap:16px; padding:12px 16px; background:#f8fafc; border-bottom:1px solid var(--border);">
          <div style="min-width:140px">
            <app-ss [options]="dispDiaOpts()" placeholder="Seleccione día..." [(ngModel)]="dispDia"></app-ss>
          </div>
          <div style="min-width:140px">
            <app-ss [options]="dispJornadaOpts" placeholder="Jornada..." [(ngModel)]="dispJornada"></app-ss>
          </div>

          <button class="btn btn-blue" style="height:32px; padding: 0 16px; font-size:12px;" (click)="checkDisp()">Verificar Disponibilidad</button>
        </div>

        @if (dispResult().length > 0) {
        <div class="disp-grid" style="padding:16px; background:#fff; border-bottom: 2px solid var(--border);">
          @for (a of dispResult(); track a.id) {
          <div class="disp-card" [class.ocupado]="!a.disponible" [class.disponible-click]="a.disponible"
               ...
               [title]="a.disponible ? 'Click para crear horario en ' + a.nombre : ''">
            ...
            <span class="disp-status">{{ a.disponible ? 'Disponible · Click para crear horario' : 'Ocupado' }}</span>
          </div>
          }
        </div>
        }
        ...
        <div class="matrix-cell" style="position: relative;" [class.is-today]="isDiaHoy(d)">
          ...
          <div class="slot-wrapper">
            ...
            <div class="horario-card" [class.en-curso]="isCurrentTime(h)">
              @let fichaEvs = fichaEventoMap().get(+(h.fichaId ?? h.ficha?.id ?? 0));
        ...
        @if ((tooltipState()!.comp.diasClase ?? []).length > 0) {
          <div class="tt-form-table">
            <div class="tt-form-header-row">
              ...
              <span class="tt-form-lbl">Horas: <strong>{{ calcHorasCompetencia(tooltipState()!.comp, tooltipState()!.h) }}</strong></span>
            </div>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td style="font-size:12px;">
                        <strong>{{ c.horario?.ficha?.codigo || '—' }}</strong><br>
                        <span style="color:var(--text-muted);font-size:11px;">{{ c.horario?.ficha?.programa || '' }}</span>
                      </td>
                      <td style="font-weight:600;font-size:13px;max-width:200px;">{{ c.nombre }}</td>
                      <td style="font-size:12px;color:var(--text-muted);max-width:180px;">{{ c.resultado || '—' }}</td>
                      <td style="font-size:11px;white-space:nowrap;">
                        @if (c.fechaInicio) {
    ...
    @if (histDiasPopover()) {
      <div class="hist-dias-popover"
        ...
        @for (iso of (histDiasPopover()!.c.diasClase ?? []); track iso) {
          <div class="tt-form-row">
            <span class="tt-form-dia">{{ formatDiaClaseFull(iso) }}</span>
            <span class="tt-form-hrs">{{ formatHorasPorDiaStr(histDiasPopover()!.c.horario) }} · {{ to12h(histDiasPopover()!.c.horario?.horaInicio) }} — {{ to12h(histDiasPopover()!.c.horario?.horaFin) }}</span>
          </div>
        }
      </div>
    }
  ...
  styles: [`
    ...
    .disp-grid { display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:12px;margin-top:12px; }
    .disp-card {
    ...
    .disp-status { font-size:11px;color:var(--text-muted); }
...
export class AdminHorariosComponent implements OnInit, OnDestroy {
  ...
  histFiltered = computed(() => {
    ...
    return this.histItems().filter(c =>
      ...
      (`${c.horario?.instructor?.nombre ?? ''} ${c.horario?.instructor?.apellido ?? ''}`).toLowerCase().includes(q) ||
      (c.horario?.ficha?.codigo ?? '').toLowerCase().includes(q)
    );
  });
  ...
  histByInstructor = computed(() => {
    ...
    for (const c of this.histFiltered()) {
      const id     = String(c.horario?.instructor?.id ?? 'sin');
      const nombre = `${c.horario?.instructor?.nombre ?? ''} ${c.horario?.instructor?.apellido ?? ''}`.trim() || 'Sin instructor asignado';
      if (!map.has(id)) map.set(id, { key: id, nombre, items: [] });
  ...
  getFichasDelGrupo(items: any[]): { codigo: string; programa: string }[] {
    ...
    items.forEach(c => {
      const cod = c.horario?.ficha?.codigo;
      if (cod) map.set(cod, c.horario.ficha.programa ?? '');
    });
  ...
  getItemsFiltradosPorFicha(group: { key: string; items: any[] }): any[] {
    ...
    return group.items.filter(c => c.horario?.ficha?.codigo === sel);
  }
  ...
  fichaEventoMap = computed(() => {
    ...
    this.eventos().forEach(ev => {
      ...
      (ev.fichasParticipantes ?? []).forEach((fid: number) => {
        const id = +fid;
  ...
  horariosEnriquecidos = computed(() => {
    const insts = this.instructores();
    const fics = this.fichas();
    const ambs = this.ambientes();
    return this.horarios().map((h: any) => ({
      ...h,
      instructor: insts.find(i => String(i.id) === String(h.instructorId)),
      ficha: fics.find(f => String(f.id) === String(h.fichaId)),
      ambiente: ambs.find(a => String(a.id) === String(h.ambienteId)),
    }));
  });

  tableRows = computed(() => this.buildRows(this.activeTab(), this.horariosEnriquecidos()));
  ...
  isActive(h: any): boolean {
    ...
    const instructor = h.instructor;
    return instructor?.sesionActiva === true;
  }
  ...
  descargarReporte() {
    ...
    const horariosHoy = this.horariosEnriquecidos().filter((h: any) => h.diaSemana === diaHoy);
    ...
    const rowsEv = eventosHoy.length ? eventosHoy.map((ev: any) => {
      ...
      return `<tr><td><strong>${ev.nombre}</strong></td><td>${tipo}</td><td>${hora}</td><td>${ev.ubicacionNombre ?? ev.lugar ?? '—'}</td><td style="text-align:center">${fichas}</td><td style="color:#6b7280;font-size:12px">${ev.descripcion ?? '—'}</td></tr>`;
...
<section>
  ...
  abrirWizardDesdeAmbiente(ambiente: any) {
    ...
    if (this.dispDia) {
      // area viene directamente del spread del Ambiente entity en checkDisponibilidad
      const area = ambiente.area ?? '';
  ...
  // ── Áreas disponibles en fichas (para el filtro del wizard) ────
  fichaAreas = computed(() =>
  ...
  saveWizardHorarios() {
    ...
    const diasPayload = this.wizardForm.dias.map(d => {
      ...
      return {
        ...
        horaInicio: this.wizardForm.horaInicio,
        horaFin: this.wizardForm.horaFin,
        fichaId: this.wizardForm.diasConfig[d].fichaId,
        ambienteId: transversal ? null : this.wizardForm.diasConfig[d].ambienteId,
        instructorId: this.wizardForm.diasConfig[d].instructorId,
      };
    });
  ...
  checkDisp() {
    if (!this.dispDia || !this.dispJornada) return;
    this.api.getAmbientesDisponibilidad(this.dispDia, this.dispJornada)
      .subscribe(r => this.dispResult.set(r));
  }
