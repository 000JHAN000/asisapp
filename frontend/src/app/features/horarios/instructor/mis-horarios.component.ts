        ...
        <div class="matrix-cell" style="position: relative;">
          @if (horariosByDay()[d]?.length) {
            ...
            <div class="slot-wrapper">
              ...
              <div class="horario-card" [class.active-session]="h.activo" [class.en-curso]="!h.activo && isEnCurso(d, h, now())">
                ...
                <div class="card-layout">
                  ...
                  <div class="card-main">
                    ...
                    <div class="card-top">
                      ...
                      @if (esTransversal() && !h.activo && !ambienteSeleccionado()[h.id] && !h.ambiente?.nombre && !h.ubicacionTransversalNombre && isEnCurso(d, h, now())) {
                    ...
                    <div class="card-bottom">
                      ...
                      @if (h.activo) {
                        ...
                        <div class="text-xs text-muted text-center mt-1">En curso...</div>
                      }

                      @if(!h.activo && h.motivoFinalizacion) {
    ...
    <div class="modal-overlay">
      <div class="modal comp-modal-wide" (click)="$event.stopPropagation()">
        ...
        <div class="form-group mt-3">
          ...
          <textarea class="form-control" [(ngModel)]="compForm.resultado" rows="2"></textarea>
        </div>
...
export class InstructorMisHorariosComponent implements OnInit, OnDestroy {
  ...
  histFiltered = computed(() => {
    ...
    return this.histItems().filter(c => {
      if (ficha && c.horario?.ficha?.codigo !== ficha) return false;
      if (!q) return true;
      return (
  ...
  openHistorial() {
    ...
    this.api.getCompetenciasByInstructor(id).subscribe({
      next: data => this.zone.run(() => { this.histItems.set(data); this.histLoading.set(false); this.cdr.detectChanges(); }),
  ...
  // ── Transversal ──────────────────────────────────────────────
  esTransversal = computed(() => !!(this.auth.currentUser() as any)?.esTransversal);
  ...
  constructor(private api: ApiService, public auth: AuthService, private zone: NgZone, private cdr: ChangeDetectorRef, private asistencia: AsistenciaService) {}
  ...
  ngOnInit() {
    ...
    this.timer = setInterval(() => {
      ...
      // Recarga eventos para mantener los iconos de campana actualizados en tiempo real
  ...
  cargarDatos() {
    const id = this.auth.currentUser()?.perfilId;
    if (!id) return;
      ...
      this.api.getHFichas().subscribe((fichas: any[]) => {
        this.api.getHAmbientes().subscribe((ambientes: any[]) => {
          ...
          h.forEach((hor: any) => {
            ...
            hor.ficha = fichas.find(f => String(f.id) === String(hor.fichaId));
            hor.ambiente = ambientes.find(a => String(a.id) === String(hor.ambienteId));
            byDay[hor.diaSemana].push(hor);
          });
          Object.keys(byDay).forEach(k => {
            byDay[k].sort((a, b) => a.horaInicio.localeCompare(b.horaInicio));
          });
      ...
      this.asistencia.crearSesion({ 
        ...
        error: (err: any) => {
          ...
          if (!msg.includes('Ya existe')) {
            this.toast.error('Error al crear sesión de asistencia', msg || 'No se pudo crear la sesión');
          }
        },
      });
    };
  ...
  consultarAmbientes(h: any) {
    ...
    this.api.getAmbientesDisponiblesTransversal(diaHoy, h.jornada).subscribe({
      next: list => {
        const visibles = list.filter((a: any) => a.estado !== 'activo');
  ...
  openComp(h: any) {
    ...
    this.compForm = { horarioId: h.id };
    this.compDiasClase.set([]);
    this.compCalMes.set(new Date());
  }

  saveComp() {
    const body = { ...this.compForm, diasClase: this.compDiasClase().length ? this.compDiasClase() : null };
